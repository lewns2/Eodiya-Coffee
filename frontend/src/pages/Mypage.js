...
function Mypage(props) {
    ...
    const handleSubmit = (event) => {
      ...
      const handleDelete = () => {
        const data = {email:window.localStorage.getItem("user_email"), password:password};
        alert("A name was submitted: " + data);
        axios
          .delete(
            "/accounts/profile/",
          ...
          .then((response) => {
            console.log(response, "from login");
            window.localStorage.setItem("iwt", "")
            handleCloseModal();
          })
          .catch((response) => {
            console.log("Error!");
    ...
    return (
        ...
        {!isdelmode &&
          <>
            비밀번호: <input type="text" value={password} onChange={handlePasswordChange} placeholder="이메일"></input>  
            <input type="button" onClick ={handleDelete} value = "삭제"></input>
            <input type="button" onClick ={handleIsdelmodeChange} value = "취소"></input>
          </>
        }
        </div>
    );
